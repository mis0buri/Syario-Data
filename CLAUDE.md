...
`xarchive-add.html` / `xarchive-add.css` / `xarchive-add.js` — a self-contained Xアーカイブ追加 (admin archive upload) page, fully decoupled from `index.html`'s SPA. Loads its own Firebase compat SDKs (app/auth/firestore/storage) and signs in via `signInWithPopup` (Google/Twitter, same popup-only rationale as `_doLogin()` in app.js). UI is gated on `/admins/{uid}` existing; actual write access is still enforced by the `storage.rules` UID allowlist (the page shows the logged-in UID with a copy button for registering it). Uploads to the **same** Storage layout as the in-SPA 追加 section (`archives/manifest.json` + `archives/{username}/{YYYY-MM}.json`), so archives added here appear in the SPA's アーカイブ閲覧. `_xaNormalizeTweet`/`xaUpload` are intentional copies of the versions in `xarchive.js` — when changing the parse/upload logic, keep both files in sync manually. The SPA's `#sec-admin-xarchive-add` links to this page.

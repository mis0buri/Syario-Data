...
`archive-upload/index.html` / `xarchive-add.css` / `xarchive-add.js` — a self-contained Xアーカイブ追加 (admin archive upload) page served at the clean URL `/archive-upload/` (the HTML lives in `archive-upload/`, referencing the root-level `xarchive-add.css`/`xarchive-add.js` via `../` paths), fully decoupled from `index.html`'s SPA. Loads only the Firebase app/storage compat SDKs and has **no login** — access control is deferred entirely to `storage.rules` (currently permitting unauthenticated uploads; if that changes, re-add auth to the page). Uploads to the **same** Storage layout as the in-SPA 追加 section (`archives/manifest.json` + `archives/{username}/{YYYY-MM}.json`), so archives added here appear in the SPA's アーカイブ閲覧. Unlike the SPA version there are no ハンドル名/表示名 inputs: the user selects `data/account.js` together with the tweets files, and username/display name are auto-extracted from its `{account: {username, accountDisplayName}}` entry (upload is rejected if no account.js is present). `_xaNormalizeTweet` is an intentional copy of the version in `xarchive.js` (keep in sync manually); `xaUpload` here has diverged from the SPA's (account.js auto-detection instead of form inputs). In **both** `xaUpload` implementations the existing manifest is read (with `cache: 'no-store'`) *before* uploading chunks, and any read failure other than `storage/object-not-found` aborts the upload — falling back to an empty manifest would overwrite it and drop previously registered accounts. The SPA's `#sec-admin-xarchive-add` links to this page.
...
3. `stats.js`, `schedule-ui.js`, `board.js`, `vote.js`, `column.js`, `gallery.js`, `renban.js`, `boshu.js`, `stamp.js`, `admin.js`, `swarm.js`, `transit.js`, then `iidx-data.js` before `iidx.js` (seed table global)
...
| **xarchive.js** | Xアーカイブ — viewer is the gallery section `#sec-xarchive`(Twitterアーカイブ, `initXArchive()`, **login-gated**: `_currentUser` required, guard message + hidden body when logged out, re-evaluated on auth resolve/logout via `updateAuthUI`); upload stays the admin section `#sec-admin-xarchive-add`(追加, gated by `_isAdmin`). The account-chip empty placeholder（アカウントがありません）only shows after a manifest load (`.xa-accounts.loaded:empty::after`; `.loaded` added by `_xaRenderAccountChips`). Stores parsed X archives in **Firebase Storage** (`_storage`, compat SDK): `archives/manifest.json` + `archives/{username}/{YYYY-MM}.json` (month chunks). Upload parses `tweets.js` in-browser (`_xaNormalizeTweet`), extracting only text/date/type/media-URL (DMs etc. never read). Viewer merges accounts into one chronological timeline with account/date filters + AND search, incremental render via IntersectionObserver, IndexedDB chunk cache (`xarchive` DB, invalidated by manifest `generated_at`). Post cards show an avatar (`_xaAvatarHtml`): current X icon via `unavatar.io/twitter/{username}?fallback=false`, falling back to an initials circle (hue from username hash) when the fetch 404s/fails (`onerror` removes the overlay `<img>`). Media referenced by pbs.twimg.com URL (not stored). Post cards have a リンクをコピー button (`xaCopyPostUrl`); admins additionally see a 脱法 button (same function with `datsuho=true`) that copies the URL minus its first character (`ttps://…`) to dodge link auto-expansion — UI-only `_isAdmin` gate at card render time. The login gate is **UI-only** — Storage-side access control is left to the deployed `storage.rules` (repo file: **UID allowlist**, unchanged by the gallery move; Storage rules can't read `/admins`, so admins add their UID and `firebase deploy --only storage`). The viewer reads chunks via `fetch()` of the download URL, so the bucket **needs a CORS policy** allowing the site origin (`https://mis0buri.github.io`) — without it, uploads succeed but the viewer shows "アーカイブがまだありません" (CORS-blocked fetch). Apply once with `gsutil cors set cors.json gs://syariodate.firebasestorage.app` (config kept in repo-root `cors.json`); bucket-level, no site redeploy needed. Media (`<img>`/`<video>` tags) is CORS-exempt |
| **iidx.js** | 弐寺ランプ — beatmania IIDX SP☆12 lamp checklist (`#sec-iidx`, nav button + section open to **any logged-in user**; the リザルト読取 and 表を更新 buttons (`#iidx-ocr-toggle-btn`/`#iidx-import-toggle-btn`) stay gated to `_isAdmin \|\| _isManager` because they need `admin_secrets` API keys / `iidx_config` write, UI-only gate backed by firestore.rules). 他ユーザーの記録を閲覧 (`#iidx-others-btn` → `iidxToggleOthers()`): lists all `iidx_lamps` docs (readable by any authed user) excluding self and users with an empty `lamps` map (= all NO PLAY, since noplay deletes the key), joined with `/users/{uid}.displayName`; selecting one switches the whole table to that user (`_iidxViewUid`/`_iidxViewLamps`, all rendering **and editing** via `_iidxCurLamps()` — lamps are intentionally editable by anyone and save to the *viewed* user's doc; リザルト読取/表を更新 hidden via `_iidxUpdateManageBtns()`) with a banner + 「自分の記録に戻る」 button. The debounced save captures target uid+lamps at schedule time (`_iidxSavePending`) and view switches call `_iidxFlushSave()` first, so a pending save can't land on the wrong user's doc. Song tier table: Firestore `iidx_config/table` if present, else static `IIDX_SEED_TABLE` from `iidx-data.js`; 「表を更新」 offers two paths: 「wikiから自動取得」 (`iidxAutoFetchTable` — first fetches the page HTML via the Cloud Functions proxy `fetchIidxWiki` (`functions/index.js`; atwiki blocks browser CORS so the Function fetches server-side and returns the raw HTML) and parses the displayed text client-side (`_iidxFetchWikiViaProxy`/`_iidxParseWikiHtml`/`_iidxParseWikiLines`), accepted only when the result has ≥10 tiers and ≥300 songs total; `fetchIidxWiki` caches the last successful HTML to Cloud Storage (`cache/iidx-wiki.html`) and, when the live atwiki fetch fails (atwiki intermittently blocks GCP IPs), serves that cache instead; a scheduled warmer `refreshIidxWikiCache` (every 4h) retries the live fetch so the cache stays populated through block windows — signaled to the client via the `X-Iidx-Source: cache` response header (`_iidxFetchWikiViaProxy` returns `{ tiers, cached }`), which `iidxAutoFetchTable` surfaces in the save-confirm label (「wiki自動取得（直接読込・wiki一時不通のため前回取得分）」); otherwise falls back to the previous approach — Gemini `url_context` tool reads the atwiki page directly, since grounding tools forbid `responseMimeType` so JSON is sliced out of the text response by `_iidxParseTiersJson`) or manual paste (tier-header line + song lines, tab/／-separated, `_iidxParseImport`). Both go through `_iidxSaveTableWithConfirm`, which warns when lamped songs would vanish from the new table and reports an added/removed diff on success. The seed includes a 「未定」 tier (rank-undetermined songs); tiers with zero songs render an empty 「曲なし」 frame only when no search/lamp filter is active. Lamps stored per-user in `iidx_lamps/{uid}` as `{ lamps: { songTitle: code } }` — the **whole doc is overwritten** on save (debounced 500ms) because song titles can contain `.`, which breaks Firestore field-path partial updates. Song title is the lamp key: renaming titles in the table orphans existing lamps. Lamp levels: noplay/failed/assist/easy/clear/hard/exhard/fc (`IIDX_LAMP_LEVELS`); tier progress counts EASY+ as クリア. リザルト読取: uploads result screenshots → client-side downscale to JPEG → Gemini Vision (`gemini-2.5-flash`, JSON response mode) extracts `{title, level, lamp}` → bigram-Dice fuzzy match against table titles (`_iidxMatchSong`, threshold 0.5) → user confirms per-row (song/lamp editable, default-checked only when matched + ☆12-or-unknown + lamp upgrade) before `iidxSetLamp` applies. Reuses admin.js's Gemini key round-robin (`_aiDiscApiKeys`/`_nextGeminiKey`/`_throwApiError`; loads `admin_secrets/api_keys` itself if the AI議論 section hasn't) — this is why managers have read access to `admin_secrets` |
| **iidx-data.js** | Static `IIDX_SEED_TABLE` seed for the ☆12 tier table (loaded before iidx.js) |
| **schedule.js** | Static `SCHEDULE_DATA` object only |
| **push.js** | 予約プッシュ通知 — FCM Web Push opt-in (マイページ button → SW register + token to Firestore `fcm_tokens/{token}`); requires `FCM_VAPID_KEY` (public, committed) and `functions/` deploy. SW: `firebase-messaging-sw.js` (config inlined; registered with explicit path because the site lives under /Syario-Data/). One-time feature announcement (app.js): NEW badge on the nav マイページ button + speech bubble above the push button in マイページ; cleared on first マイページ open via localStorage `push_notice_seen`, and never shown when `push_enabled` is already `1` |
...
| `iidx_config` (`/table`) | Admin or Manager (read: any authed) — ☆12 tier table override |
| `iidx_lamps` | Any authed user may write **any** user's doc (intentional: club members maintain each other's records via 他ユーザー閲覧) — per-user IIDX lamp records |
...
- **Xアーカイブ upload success** → same Discord webhook as feedback, from both `xaUpload` implementations (app.js's `WEBHOOK_URL` in xarchive.js; standalone page duplicates it as `XAA_WEBHOOK_URL` in xarchive-add.js — update both when rotating the URL). Notification failure never fails the upload (try/catch + console.warn)
- **First-time login** → separate Discord webhook (deduplicated via localStorage key `syario_loggedin_{uid}`). Fires for **both** Google and X via `_doLogin` → `_notifyDiscordLogin`; the fetch is `await`ed and sent with `keepalive: true` so the `location.reload()` right after login can't abort it (the dedup key is set before sending, so an aborted request would otherwise be lost forever)

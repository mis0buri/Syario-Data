...
// ── 境界駅リナンバリング判定（ラウンド6/7/8の全数検証で確定した4方向の番号規則を統一） ──
// JR側の列車番号形式: 数字+サフィックス1文字、プレフィックス無し（例:'793S' '1770A'）。
// メトロ/TR側の列車番号形式: 先頭に任意で1文字のプレフィックス+数字+サフィックス1文字+
// 末尾に任意でR（例:'A793SR' 'B1949S' 'A605K'）。中野forward（ラウンド6/7、71/71件）は
// プレフィックス'A'固定だったが、西行き系（ラウンド8）では'B'プレフィックスが観測された
// ため、プレフィックスは任意の1文字として吸収する（数字とサフィックスの対応関係が本質）。
const _RE_TR_JR_NO = /^(\d+)([A-Za-z])$/;
const _RE_TR_METRO_NO = /^[A-Za-z]?(\d+)([A-Za-z])R?$/;

// JR側番号(noJR)とメトロ側番号(noMetro)が「同一物理列車が境界駅で番号・サフィックスを
// 変えて直通継続している」規則に合致するか判定する。offsetは(メトロ番号-JR番号)の差分
// （中野forward/西船橋東行き=+1、中野西行き/西船橋西行き=±0）。jrSuf/metroSufは各側で
// 許容するサフィックス文字（大文字）の配列。メトロ側に対応便が存在しない/サフィックス
// 不一致の場合は単に不発火となるだけで、既存の通常乗換ロジック（条件c等）に影響しない。
function _trBoundaryRenumber(noJR, noMetro, offset, jrSuf, metroSuf) {
  if (!noJR || !noMetro) return false;
  const mJR = _RE_TR_JR_NO.exec(noJR);
  if (!mJR) return false;
  if (!jrSuf.includes(mJR[2].toUpperCase())) return false;
  const mMetro = _RE_TR_METRO_NO.exec(noMetro);
  if (!mMetro) return false;
  if (!metroSuf.includes(mMetro[2].toUpperCase())) return false;
  return (parseInt(mMetro[1], 10) - parseInt(mJR[1], 10)) === offset;
}

// 境界駅ごとの直通番号規則テーブル。station=境界駅名（正規化済み）、aKind=leg a
// （境界より前）がJR側かメトロ側か、hsCheck=leg aのheadsignが規則の対象か
// （★誤結合防止の一次条件。中野西行きでheadsign='中野行'（額面通りの終着）のTozai legを
// 対象外にする等、番号一致だけでは判別できない例外を防ぐ）。全数検証の実証件数は
// FINDINGS.mdラウンド6/7/8参照（71/71・81/81・7/7・11/11）。
const TRANSIT_BOUNDARY_RULES = [
  { // 中野forward（東行き、JR「中野」headsign→メトロ+1・S/K）ラウンド6/7、71/71実証
    station: '中野', aKind: 'JR',
    hsCheck: hs => _trNorm(hs) === '中野',
    offset: 1, jrSuf: ['A', 'Y'], metroSuf: ['S', 'K'],
  },
  { // 中野西行き（東西線「Mitaka行」headsign→JR±0・A/Y/B）ラウンド8、81/81実証
    station: '中野', aKind: 'Metro',
    hsCheck: hs => _trHsCanon(hs) === '三鷹',
    offset: 0, jrSuf: ['A', 'Y', 'B'], metroSuf: ['S', 'K'],
  },
  { // 西船橋東行き（東西線「Tsudanuma行」headsign→JR津田沼方面+1・Y）ラウンド8、7/7実証
    station: '西船橋', aKind: 'Metro',
    hsCheck: hs => _trHsCanon(hs) === '津田沼',
    offset: 1, jrSuf: ['Y'], metroSuf: ['S', 'K'],
  },
  { // 西船橋西行き（JR「西船橋」headsign+suffix Y→メトロ±0・S/K）ラウンド8、11/11実証。
    // suffix='A'の西船橋行きは対応するメトロ便が無い(6/6)ため、jrSufにAを含めない
    station: '西船橋', aKind: 'JR',
    hsCheck: hs => _trNorm(hs) === '西船橋',
    offset: 0, jrSuf: ['Y'], metroSuf: ['S', 'K'],
  },
];

// leg a（境界より前）→leg b（境界より後）が上記いずれかの規則で同一物理列車と
// 判定できるか判定する。station不一致（そもそも隣接境界ではない）なら即false
function _trBoundaryThroughMatch(a, b) {
  const station = _trNorm(a.to.name);
  if (!station || station !== _trNorm(b.from.name)) return false;
  for (const rule of TRANSIT_BOUNDARY_RULES) {
    if (rule.station !== station) continue;
    if (!rule.hsCheck(a.headsign)) continue;
    const jrNo = _trTrainNo(rule.aKind === 'JR' ? a.tripId : b.tripId);
    const metroNo = _trTrainNo(rule.aKind === 'JR' ? b.tripId : a.tripId);
    if (_trBoundaryRenumber(jrNo, metroNo, rule.offset, rule.jrSuf, rule.metroSuf)) return true;
  }
  return false;
}
...
// pairはTRANSIT_THROUGH_PAIRSと同じ「路線名の部分文字列同士のペア」形式（他コードとの
// 対応関係を示す資料的な意味で維持）。対象はfast_report.txt T4のコリドー計算・
// fast_boundary_coords.jsonで座標確認済みの8駅のみ（西船橋・北千住は既存TRANSIT_HUBSで
// 静的にカバー済みのため対象外、白金高輪は座標未確認のため今回は対象外）。★idは必ず
// geoクラスタID形式を使う。中野を東西線の正準駅ID(odpt-tokyo-metro-part-01:
// odpt.Station:TokyoMetro.Tozai.Nakano)で指定すると「快速→中野→東西線」パターンが
// 1件も出現しないことが確認済み(fast_report.txt T1(b'))。
// trigger: 課題F2(ラウンド16)で追加。実際に発火条件として使うのはこちらで、
// 「直通先（地下鉄/私鉄側）の路線名」の配列。旧実装はpairの両側（JR側含む）のいずれかを
// resultプールのlegが含んでいれば発火する広い条件だったため、例えば中央線快速のみで
// 完結する経路でもvia=中野が発火し得た。trigger化により「直通先の路線を使うlegが実際に
// プールに存在するときだけ」発火するよう厳格化する（配列なのは1駅に複数の直通先路線
// パターンがある場合にorで複数指定できるようにするため。現状は各エントリ1本のみ指定して
// いるが、和光市・小竹向原・目黒のように同じ駅名で複数エントリに分かれているケースは、
// エントリ単位でtriggerが異なる＝実質的に駅名単位でのorになっている）
const TRANSIT_THROUGH_BOUNDARIES = [
  { pair: ['東西線', '中央'],     trigger: ['東西線'],   name: '中野',       lat: 35.70614607142857,  lon: 139.66516622916666,  id: 'geo:35.706146,139.665166' },
  { pair: ['東西線', '総武'],     trigger: ['東西線'],   name: '中野',       lat: 35.70614607142857,  lon: 139.66516622916666,  id: 'geo:35.706146,139.665166' },
  { pair: ['千代田線', '小田急'], trigger: ['千代田線'], name: '代々木上原', lat: 35.66904089285714,  lon: 139.67972650000002,  id: 'geo:35.669041,139.679727' },
  { pair: ['日比谷線', '東急東横'], trigger: ['日比谷線'], name: '中目黒',   lat: 35.64413718174343,  lon: 139.6988560499589,   id: 'geo:35.644137,139.698856' },
  { pair: ['副都心線', '東武東上'], trigger: ['副都心線'], name: '和光市',   lat: 35.78816173999999,  lon: 139.61244566750003,  id: 'geo:35.788162,139.612446' },
  { pair: ['有楽町線', '東武東上'], trigger: ['有楽町線'], name: '和光市',   lat: 35.78816173999999,  lon: 139.61244566750003,  id: 'geo:35.788162,139.612446' },
  { pair: ['浅草線', '京成'],     trigger: ['浅草線'],   name: '押上',       lat: 35.709520971124626, lon: 139.81342929939208,  id: 'geo:35.709521,139.813429' },
  { pair: ['浅草線', '京急'],     trigger: ['浅草線'],   name: '泉岳寺',     lat: 35.63841257730264,  lon: 139.73936017463237,  id: 'geo:35.638413,139.739360' },
  { pair: ['三田線', '目黒線'],   trigger: ['三田線'],   name: '目黒',       lat: 35.63344893785311,  lon: 139.71576133050849,  id: 'geo:35.633449,139.715761' },
  { pair: ['三田線', '東急目黒'], trigger: ['三田線'],   name: '目黒',       lat: 35.63344893785311,  lon: 139.71576133050849,  id: 'geo:35.633449,139.715761' },
  { pair: ['南北線', '目黒線'],   trigger: ['南北線'],   name: '目黒',       lat: 35.63344893785311,  lon: 139.71576133050849,  id: 'geo:35.633449,139.715761' },
  { pair: ['副都心線', '西武'],   trigger: ['副都心線'], name: '小竹向原',   lat: 35.74338070588236,  lon: 139.6795244338235,   id: 'geo:35.743381,139.679524' },
  { pair: ['有楽町線', '西武'],   trigger: ['有楽町線'], name: '小竹向原',   lat: 35.74338070588236,  lon: 139.6795244338235,   id: 'geo:35.743381,139.679524' },
];
...
function _trMergeThroughLegs(j) {
  ...
  const isThrough = (a, b, walkSecs) => {
    ...
    // 境界駅（中野・西船橋）でのリナンバリング直通規則（ラウンド6/7/8で確定、4方向）。
    // 番号+サフィックス+headsignガードが揃えば同一物理列車として無条件結合する
    if (_trBoundaryThroughMatch(a, b)) return true;
...
// transit legのrouteNameを走査し、TRANSIT_THROUGH_BOUNDARIESの各エントリのtrigger
// （直通先＝地下鉄/私鉄側の路線名）にマッチする路線を持つ境界駅を収集する（課題F2、
// ラウンド16で厳格化。旧実装はpairの両側＝JR側も含めて判定していたため、中央線のみで
// 完結する経路でもvia=中野が発火し得る広いトリガーだった。trigger化により「直通先の
// 路線を使うlegが実際に結果プールに存在するときだけ」発火するよう限定した）。
// マッチした境界駅のうち、
...
function _trBoundaryViaFanout(pool, token, avoid, detour, pr, silent) {
  ...
  pool.all.forEach(j => (j.legs || []).forEach(l => {
    ...
    TRANSIT_THROUGH_BOUNDARIES.forEach(b => {
      ...
      // 課題F2: 発火条件は「直通先(地下鉄/私鉄側)の路線」の存在のみ（b.trigger）。
      // pair（JR側含む両側）はもう発火条件に使わない
      if (!b.trigger.some(t => l.routeName.includes(t))) return;
